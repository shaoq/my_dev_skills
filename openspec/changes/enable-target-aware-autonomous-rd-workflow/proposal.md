...
当 Runtime 通过不可由模型、仓库文件、环境变量或嵌套 skill 调用伪造的 dispatch provenance，证明用户显式调用 Claude `/new-worktree-apply ...`、Codex `$new-worktree-apply ...` 或等价受支持命令时，该调用即授权在规范化隔离 worktree 中创建来源分支、执行 apply 并提交来源变更。安全性由可信显式调用来源、确定性参数、只读预检、冻结快照和写前漂移复检共同提供，而不是依赖 Issue、Team、任务平台 envelope 或重复确认。
...
- 增加跨 Runtime 的 `explicit-skill-invocation/v1` dispatch provenance 契约。Claude slash command、Codex `$skill` 或其他 Runtime 的等价显式入口必须由 Runtime 可信元数据证明并绑定本次 skill、原始参数和 invocation；模型自动选择、自然语言推断、嵌套 skill 转调、用户粘贴的伪造 metadata 或来源 unknown 均在任何写入前停止。
...
- 保留冻结目标、artifact manifest、canonical identity、目标 worktree clean、最终漂移复检、失败关闭和现场保留等现有安全不变量。默认执行在首次写入前发现任何漂移时直接停止，不自动刷新快照、换目标或降级为交互确认。
...
- 修改 `new-worktree-apply/SKILL.md` 的参数契约、可信显式调用 provenance、默认执行语义、dry-run、写前复检、报告与 guardrails；删除 Issue/Team/task-platform 授权协议和人工确认路径。
...
- 扩展 `tests/worktree-lifecycle-safety.sh`，覆盖 Claude/Codex 显式 dispatch、模型自动或嵌套调用拒绝、默认直接执行、`--dry-run` 零写、旧授权参数拒绝、缺失显式 target、双侧快照漂移和确认边界隔离。
