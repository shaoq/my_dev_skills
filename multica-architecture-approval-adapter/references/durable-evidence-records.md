...
Every mapping, readiness or decision sidecar write requires a current `operational_authorization` whose `Exact planned writes` names the final and temporary relative paths, confirmed existing scope, no-clobber publish, reread and retained objects. An authority for one record kind/path does not cover another. If a final object already exists with different bytes or a partial attempt changed the retained set, stop and request a new conflict/retry authorization; never infer overwrite or repair permission.
