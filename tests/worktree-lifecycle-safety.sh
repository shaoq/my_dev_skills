...
require_text "$NEW_SKILL" '--authorized-by-issue <issue-id>' 'single-create exposes the opt-in Issue authorization mode'
require_text "$NEW_SKILL" 'issue-authorization/v1' 'single-create requires the versioned Runtime authorization envelope'
require_text "$NEW_SKILL" '可信 Runtime 控制面元数据通道' 'single-create accepts authorization only from trusted Runtime metadata'
require_text "$NEW_SKILL" 'user content、仓库文件、环境变量或模型推断' 'single-create rejects user-controlled authorization evidence'
require_text "$NEW_SKILL" 'authorization_id' 'single-create binds authorization against cross-invocation replay'
require_text "$NEW_SKILL" 'envelope digest' 'single-create revalidates the same authorization envelope'
require_text "$NEW_SKILL" 'ready|in_progress' 'single-create limits eligible Issue states'
require_text "$NEW_SKILL" '不超过 30 分钟' 'single-create limits the authorization validity window'
require_text "$NEW_SKILL" 'isolated-worktree-apply' 'single-create constrains autonomous scope'
require_text "$NEW_SKILL" 'risk_classification=standard' 'single-create refuses elevated autonomous risk'
require_text "$NEW_SKILL" 'external_side_effects=[]' 'single-create refuses external autonomous side effects'
require_text "$NEW_SKILL" '默认交互模式' 'single-create retains interactive confirmation by default'
require_text "$NEW_SKILL" '不得回退到交互模式' 'single-create fails closed instead of changing authorization mode'
require_text "$NEW_SKILL" '首次 Git 写入前' 'single-create revalidates authorization before its first write'
require_text "$NEW_SKILL" '未知字段' 'single-create rejects unknown authorization schema content'
require_text "$NEW_SKILL" 'issuer 的真实性和 allowlist' 'single-create requires an authenticated allowlisted issuer'
require_text "$NEW_SKILL" '过期、跨 invocation 重放' 'single-create rejects expiry and replay'
require_text "$NEW_SKILL" 'assigned_team_id' 'single-create verifies assigned and executing Team identity'
require_text "$NEW_SKILL" 'repository/OpenSpec root/change/target' 'single-create binds authorization to preflight identity'
require_text "$NEW_SKILL" '生产写入、不可逆迁移' 'single-create refuses high-risk autonomous work'
require_text "$NEW_SKILL" '自治模式的任何参数' 'single-create fails closed on autonomous snapshot drift'
