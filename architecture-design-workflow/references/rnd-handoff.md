...
要求准确版本的 `approved_for_spec`、已发布 ADR/详细设计，以及已确认的目标项目和仓库。缺一项就保持 `waiting_human` 或 `publishing`，不得创建授权性 handoff。

## Handoff content

- Architecture Issue、批准证据、`ARCH-DESIGN`/`ARCH-REVIEW`/ADR/详细设计版本
